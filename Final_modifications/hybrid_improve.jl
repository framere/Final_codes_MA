...
function davidson(
    ...
    iter = 0
    # convergence_tracker: Dict{Int, (λ, count, residual_history, xvec)}
    convergence_tracker = Dict{Int, Tuple{Float64, Int, Vector{Float64}, Vector{T}}}()

    # Helper function: stagnation check (uses residual history from tracker)
    function is_stagnating(residuals::Vector{Float64}; tol=0.1, window=2)
        length(residuals) < window && return false
        r_old, r_new = residuals[end-window+1], residuals[end]
    ...
    while nevf < l_buffer
        ...
        for (sorted_i, original_i) in enumerate(sorted_indices[1:current_cutoff])
            ...
            # Update convergence tracking with residual history
            if haskey(convergence_tracker, original_i)
                λ_prev, count, res_hist, _ = convergence_tracker[original_i]
                push!(res_hist, rnorm)
                if length(res_hist) > 3
                    popfirst!(res_hist) # keep short history
                end

                if abs(λ - λ_prev) < 1e-5 && rnorm < adaptive_thresh
                    convergence_tracker[original_i] = (λ, count + 1, res_hist, X_sorted[:, sorted_i])
                else
                    convergence_tracker[original_i] = (λ, 1, res_hist, X_sorted[:, sorted_i])
                end
            ...
            # Check if converged
            if haskey(convergence_tracker, original_i)
                λ, count, res_hist, xvec = convergence_tracker[original_i]
