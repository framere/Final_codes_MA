...
function correction_equations_exact_dense(A, U, lambdas, R)
    n, k = size(U)
    S = zeros(eltype(A), n, size(R,2))

    # projector P = I - U*U'
    # P = I - U * U'
    # optionally convert to Matrix if using sparse/other types:
    # P = Matrix(P)
    ...
    for j in 1:size(R, 2)
        ...
        # build M = P*(A - λ I)*P
        M = (Matrix(A) - λ * I)         # dense matrices
        rhs = - P * r

        # factorize and solve (factorize chooses LU/Cholesky as appropriate)
        F = factorize(M)
        s = F \ rhs

        # enforce P-orthogonality numerically
        s .= s - U * (U' * s)
...
function correction_equations_exact_reduced(A, U, lambdas, R)
    n, k = size(U)
    p = size(R, 2)
    S = zeros(eltype(A), n, p)

    # Build a full Q (n×n) from QR of U, then take the orthonormal complement W
    # qr(U, Val(true)) yields the full Q factor (n×n)
    Qfull = Matrix(qr(U, Val(true)).Q)   # full orthonormal basis
    if k >= n
        error("U has full column rank equal to n; no orthogonal complement available.")
    end
    W = Qfull[:, k+1:end]                # W is n × (n-k), orthonormal and orthogonal to U

    # Precompute A in dense form if needed
    Ad = Matrix(A)
    ...
    for j in 1:p
        ...
        # Reduced matrix and rhs
        # M_hat = W' * (A - λ I) * W  => size (n-k) × (n-k)
        M_hat = W' * (Ad - λ * I) * W
        rhs_hat = - W' * r         # because W' * (I - U U') r = W' r (W' * U = 0), so -W'*r

        # Solve reduced system
        # You can use factorize for numerical stability
        F = factorize(Matrix(M_hat))
        y = F \ rhs_hat

        s = W * y
        # Enforce orthogonality numerically (should already hold)
        s .= s - U * (U' * s)
...
function davidson(
    ...
    while nevf < l_buffer
        ...
        if iter < 10
            ...
            # --- JD corrections ---
            t_jd = correction_equations_exact_reduced(
                A,
                X_nc[:, jd_indices],
                Σ_nc[jd_indices],
                R_nc[:, jd_indices]
