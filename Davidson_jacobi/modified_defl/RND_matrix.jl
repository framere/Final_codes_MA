...
function occupied_orbitals(number::Int)
    if number % 2 == 0
        return 19
    else
        return 10
...
function load_matrix(filename::String, number::Int)
    if number % 2 == 0
        N = 20000
    elseif number % 2 == 1
        N = 10000
...
function davidson(
    ...
    while nevf < l_buffer
        ...
        conv_indices = Int[]
        # --- Simplified: immediate locking when rnorm < adaptive_thresh ---
...
function main(number::Integer, l::Integer, beta::Integer, factor::Integer, max_iter::Integer)
    global NFLOPs
    NFLOPs = 0  # reset for each run

    filename = "../../test/large_sparse_matrix_$number.dat"

    Nlow = max(round(Int, 0.1*l), 16)
    Naux = Nlow * beta
    A = load_matrix(filename, number)
    N = size(A, 1)
...
for number in numbers
    ...
    for beta in betas
        ...
        for (i, l) in enumerate(ls)
            nev = l * occupied_orbitals(number)
