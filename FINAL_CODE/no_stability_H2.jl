...
function davidson(
    ...
    n_b = size(V, 2)
    l_buffer = round(Int, l * 1.95)
    lc = round(Int, 1.1 * l)  # We want to converge smallest lc eigenvalues
    ...
    while nevf < l_buffer
        ...
        for (sorted_i, original_i) in enumerate(sorted_indices[1:current_cutoff])
            ...
            # Check if converged (simplified version)
            if rnorm < adaptive_thresh
                push!(conv_indices, original_i)
                push!(Eigenvalues, λ)
                Ritz_vecs = hcat(Ritz_vecs, X_sorted[:, sorted_i])
                V_lock = hcat(V_lock, X_sorted[:, sorted_i])
                nevf += 1
