...
function davidson(
    ...
    V::Matrix{T},
    n_aux::Integer,
    lambda_threshold::Float64,  # Changed from lambda_max to be more clear
    thresh::Float64,
    system::String = "",
    ...
    while true
        ...
        for i = 1:size(R, 2)
            ...
            # Skip eigenvalues that don't meet our threshold (λ < -lambda_threshold)
            if λ > -lambda_threshold
                continue
            end
            ...
            if haskey(convergence_tracker, i)
                ...
                if count >= stable_thresh && λ < -lambda_threshold
                    push!(conv_indices, i)
                    push!(Eigenvalues, λ)
        ...
        # Stop when no more eigenvalues below threshold
        if all(Σ .> -lambda_threshold)
            println("All eigenvalues with |λ| > $lambda_threshold found.")
            break
        end

        # Only consider non-converged eigenvalues below threshold for corrections
        non_conv_indices = findall(Σ .< -lambda_threshold)
...
function main(system::String, lambda_threshold::Float64, factor::Integer)
    global NFLOPs
    ...
    println("Davidson (finding eigenvalues with |λ| > $lambda_threshold)")
    @time Σ, U = davidson(A, V, Naux, lambda_threshold, 1e-2, system, 3)
...
# Example usage - find eigenvalues with |λ| > 2 (λ < -2)
lambdas = [2.26, 1.89, 1.42]

for (i, λ) in enumerate(lambdas)
    println("Running for λ = $λ")
    main("hBN", λ, i + 1)  # Replace F with loop index i + 1
end
